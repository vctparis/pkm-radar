...
export default function CardTracker() {
  ...
  void gradeOf;
  // Échelle des grades observée sur le marché, du plus haut au plus bas.
  ...
  return (
    <div>
      ...
      {selected && set ? <>
        <section className="grid gap-9 py-12 lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-14">
          ...
          <div>
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              <article className="md:col-span-2 border-t-2 border-accent pt-5"><p className="m-0 text-[0.72rem] uppercase tracking-[0.12em] text-mist-500">{marketLabel}</p>{market?.rawFR ? <><div className="mt-3 flex flex-wrap items-end gap-x-4 gap-y-2"><strong className="tabular text-[2.7rem] leading-none text-mist-050">{eur.format(market.rawFR.bestAsk ?? market.rawFR.floor10)}</strong><span className="text-[0.8rem] text-mist-300">l&apos;offre la moins chère{market.rawFR.bestAskCountry && market.rawFR.bestAskCountry !== "FR" ? ` · expédiée de ${market.rawFR.bestAskCountry}` : ""}{market.rawFR.bestAskShipping ? ` · +${eur.format(market.rawFR.bestAskShipping)} de port` : ""}</span><span className={`rounded-full border px-2 py-1 text-[0.68rem] ${confidenceClass(market.rawFR.confidence)}`}>confiance {market.rawFR.confidence}</span></div><p className="m-0 mt-3 text-[0.8rem] leading-6 text-mist-300">Le carnet monte ensuite : <strong className="text-mist-100">{eur.format(market.rawFR.floor10)}</strong> au 10<sup>e</sup> centile, <strong className="text-mist-100">{eur.format(market.rawFR.median)}</strong> en son milieu — sur {market.rawFR.offers} offres actives auprès de {market.rawFR.sellers} vendeurs. Un acheteur paie le bas du carnet ; le milieu décrit les demandes, pas un prix.</p></> : <><p className="m-0 mt-4 text-[1.35rem] text-mist-100">Pas encore de marché {languageTag} assez comparable</p><p className="m-0 mt-3 text-[0.8rem] leading-6 text-mist-500">La carte reste consultable grâce au repère européen, mais aucun prix {languageTag} EX+ n’est promu.</p></>}</article>
              <article className="border-t border-ink-500 pt-5"><p className="m-0 text-[0.72rem] uppercase tracking-[0.12em] text-mist-500">Repère Cardmarket</p>{market?.cardmarketGuide ? <><p className="tabular m-0 mt-3 text-[1.75rem] font-semibold">{market.cardmarketGuide.trend != null ? eur.format(market.cardmarketGuide.trend) : "—"}</p><p className="m-0 mt-1 text-[0.76rem] text-mist-300">tendance des ventes</p>{market.cardmarketGuide.low != null && <p className="m-0 mt-3 text-[0.78rem] text-mist-300">Offre la moins chère de leur carnet : <strong className="text-mist-050">{eur.format(market.cardmarketGuide.low)}</strong></p>}<p className="m-0 mt-3 text-[0.7rem] leading-5 text-mist-500">Guide public Cardmarket, toutes conditions et toutes langues — le carnet le plus profond du marché, absent de nos cotations. Contrepoint, jamais fusionné.</p></> : <><p className="tabular m-0 mt-3 text-[1.75rem] font-semibold">{reference ? eur.format(reference.price) : eur.format(selected.price)}</p><p className={`m-0 mt-2 text-[0.76rem] ${referenceChange != null && referenceChange > 0 ? "text-[#63c29f]" : referenceChange != null && referenceChange < 0 ? "text-[#e3bd6a]" : "text-mist-500"}`}>{referenceChange == null ? "historique indisponible" : `${referenceChange >= 0 ? "+" : ""}${referenceChange.toFixed(1)} % vs moyenne 30 j`}</p><p className="m-0 mt-3 text-[0.7rem] leading-5 text-mist-500">Aucun produit Cardmarket apparié sans ambiguïté pour cette carte : repère de plateforme, pas une vente confirmée.</p></>}</article>
            </div>
        ...
        {market?.flow && !market.flow.suspectEmptyCrawl && market.flow.exits > 0 ? <section className="border-t border-ink-600 py-10" aria-labelledby="flow-title"><p className="m-0 text-[0.76rem] uppercase tracking-[0.14em] text-accent">Rotation</p><h2 id="flow-title" className="display m-0 mt-2 text-[1.8rem]">Le carnet tourne-t-il&nbsp;?</h2><div className="mt-6 grid gap-6 md:grid-cols-[minmax(0,1fr)_22rem]"><div className="grid grid-cols-2 gap-x-8 gap-y-5 sm:grid-cols-4"><div><p className="m-0 text-[0.7rem] uppercase tracking-[0.1em] text-mist-500">Annonces actives</p><b className="tabular mt-1 block text-[1.4rem] text-mist-050">{market.flow.active}</b></div><div><p className="m-0 text-[0.7rem] uppercase tracking-[0.1em] text-mist-500">Sorties nettes</p><b className="tabular mt-1 block text-[1.4rem] text-mist-050">{market.flow.adjustedExits}</b><small className="text-[0.68rem] text-mist-500">{market.flow.likelyRelists} remise{market.flow.likelyRelists > 1 ? "s" : ""} en ligne écartée{market.flow.likelyRelists > 1 ? "s" : ""}</small></div><div><p className="m-0 text-[0.7rem] uppercase tracking-[0.1em] text-mist-500">Prix de sortie</p><b className="tabular mt-1 block text-[1.4rem] text-mist-050">{market.flow.exitPriceMedian != null ? eur.format(market.flow.exitPriceMedian) : "—"}</b><small className="text-[0.68rem] text-mist-500">médiane</small></div><div><p className="m-0 text-[0.7rem] uppercase tracking-[0.1em] text-mist-500">Présence</p><b className="tabular mt-1 block text-[1.4rem] text-mist-050">{market.flow.medianDaysListed != null ? `${market.flow.medianDaysListed} j` : "—"}</b><small className="text-[0.68rem] text-mist-500">avant disparition</small></div></div><aside className="border-l-2 border-[#c98500] pl-5 text-[0.78rem] leading-6 text-mist-300"><strong className="text-mist-050">Une sortie n&apos;est pas une vente.</strong> Une annonce peut disparaître parce qu&apos;elle est vendue, retirée ou expirée ; les remises en ligne du même vendeur sont déduites. Fenêtre observée : {market.flow.observedDays} jour{market.flow.observedDays > 1 ? "s" : ""} depuis le {shortDate.format(new Date(`${market.flow.observedSince}T12:00:00Z`))} — le délai de présence ne devient parlant qu&apos;après plusieurs semaines.</aside></div></section> : null}

        <section className="grid gap-10 border-t border-ink-600 py-12 lg:grid-cols-[minmax(0,1fr)_21rem]" aria-labelledby="grades-title"><div><p className="m-0 text-[0.76rem] uppercase tracking-[0.14em] text-accent">Gradation</p><h2 id="grades-title" className="display m-0 mt-2 text-[1.8rem]">Raw et PSA restent deux marchés.</h2><div className="mt-7 overflow-x-auto"><table className="w-full min-w-[520px] border-collapse text-left text-[0.84rem]"><caption className="sr-only">Demandes par état et par grade</caption><thead><tr className="border-y border-ink-600 text-[0.68rem] uppercase tracking-[0.08em] text-mist-500"><th scope="col" className="py-2 pr-3 font-medium">État</th><th scope="col" className="py-2 pr-3 text-right font-medium">La moins chère</th><th scope="col" className="py-2 pr-3 text-right font-medium">Milieu du carnet</th><th scope="col" className="py-2 pr-3 text-right font-medium">× la brute</th><th scope="col" className="py-2 text-right font-medium">Vendeurs</th></tr></thead><tbody><tr className="border-b border-ink-700"><th scope="row" className="py-2.5 pr-3 font-medium text-mist-050">Brute EX+ <span className="text-[0.7rem] font-normal text-mist-500">{languageTag}</span></th><td className="tabular py-2.5 pr-3 text-right text-mist-050">{market?.rawFR ? eur.format(market.rawFR.bestAsk ?? market.rawFR.floor10) : "—"}</td><td className="tabular py-2.5 pr-3 text-right text-mist-300">{market?.rawFR ? eur.format(market.rawFR.median) : "—"}</td><td className="tabular py-2.5 pr-3 text-right text-mist-500">—</td><td className="tabular py-2.5 text-right text-mist-300">{market?.rawFR?.sellers ?? "—"}</td></tr>{gradedLadder.length ? gradedLadder.map(([key, row]) => <tr key={key} className="border-b border-ink-700 last:border-b-0"><th scope="row" className="py-2.5 pr-3 font-medium text-mist-050">{row.company} {row.grade}</th><td className="tabular py-2.5 pr-3 text-right text-mist-050">{eur.format(row.bestAsk)}</td><td className="tabular py-2.5 pr-3 text-right text-mist-300">{eur.format(row.median)}</td><td className={`tabular py-2.5 pr-3 text-right ${row.multipleVsRaw && row.multipleVsRaw >= 3 ? "font-semibold text-[#63c29f]" : "text-mist-300"}`}>{row.multipleVsRaw ? `×${row.multipleVsRaw}` : "—"}</td><td className="tabular py-2.5 text-right text-mist-300">{row.sellers}</td></tr>) : <tr><td colSpan={5} className="py-4 text-[0.82rem] text-mist-500">Aucune annonce gradée relevée pour cette carte dans le dernier périmètre.</td></tr>}</tbody></table></div>{gradedLadder.length ? <p className="m-0 mt-3 text-[0.74rem] leading-5 text-mist-500">Grades 8 à 10, les seuls assez liquides pour se comparer. Demandes actives relevées sur eBay.fr, une voix par vendeur — jamais des ventes conclues. Le multiple se lit contre la carte brute du même jour ; un grade porté par un seul vendeur décrit ce vendeur, pas le marché.</p> : null}</div><aside className="border-l-2 border-[#c98500] pl-5 text-[0.8rem] leading-6 text-mist-300"><strong className="text-mist-050">Ce qui manque encore : les ventes et la population.</strong> Ce tableau montre ce qu&apos;on <em>demande</em> pour un exemplaire gradé, pas ce qu&apos;il s&apos;est <em>vendu</em>, et ignore combien d&apos;exemplaires existent dans chaque grade. Sans population, aucun ratio PSA / brute n&apos;est publié — un écart de prix sans rareté relative ne veut rien dire.</aside></section>
