  ...
  return (
    <div>
      ...
      {selected && set ? <>
        ...
        <section className="grid gap-10 border-t border-ink-600 py-12 lg:grid-cols-[minmax(0,1fr)_21rem]" aria-labelledby="grades-title"><div><p className="m-0 text-[0.76rem] uppercase tracking-[0.14em] text-accent">Gradation</p><h2 id="grades-title" className="display m-0 mt-2 text-[1.8rem]">Raw et PSA restent deux marchés.</h2><div className="mt-7 overflow-x-auto"><table className="w-full min-w-[520px] border-collapse text-left text-[0.84rem]"><caption className="sr-only">Demandes par état et par grade</caption><thead><tr className="border-y border-ink-600 text-[0.68rem] uppercase tracking-[0.08em] text-mist-500"><th scope="col" className="py-2 pr-3 font-medium">État</th><th scope="col" className="py-2 pr-3 text-right font-medium">La moins chère</th><th scope="col" className="py-2 pr-3 text-right font-medium">Milieu du carnet</th><th scope="col" className="py-2 text-right font-medium">Vendeurs</th></tr></thead><tbody><tr className="border-b border-ink-700"><th scope="row" className="py-2.5 pr-3 font-medium text-mist-050">Brute EX+ <span className="text-[0.7rem] font-normal text-mist-500">{languageTag}</span></th><td className="tabular py-2.5 pr-3 text-right text-mist-050">{market?.rawFR ? eur.format(market.rawFR.bestAsk ?? market.rawFR.floor10) : "—"}</td><td className="tabular py-2.5 pr-3 text-right text-mist-300">{market?.rawFR ? eur.format(market.rawFR.median) : "—"}</td><td className="tabular py-2.5 text-right text-mist-300">{market?.rawFR?.sellers ?? "—"}</td></tr>{gradedLadder.length ? gradedLadder.map(([key, row]) => <tr key={key} className="border-b border-ink-700 last:border-b-0"><th scope="row" className="py-2.5 pr-3 font-medium text-mist-050">{row.company} {row.grade}</th><td className="tabular py-2.5 pr-3 text-right text-mist-050">{eur.format(row.bestAsk)}</td><td className="tabular py-2.5 pr-3 text-right text-mist-300">{eur.format(row.median)}</td><td className="tabular py-2.5 text-right text-mist-300">{row.sellers}</td></tr>) : <tr><td colSpan={4} className="py-4 text-[0.82rem] text-mist-500">Aucune annonce gradée relevée pour cette carte dans le dernier périmètre.</td></tr>}</tbody></table></div>{gradedLadder.length ? <p className="m-0 mt-3 text-[0.74rem] leading-5 text-mist-500">Demandes actives relevées sur eBay.fr, une voix par vendeur — jamais des ventes conclues. Un grade porté par un seul vendeur décrit ce vendeur, pas le marché.</p> : null}</div><aside className="border-l-2 border-[#c98500] pl-5 text-[0.8rem] leading-6 text-mist-300"><strong className="text-mist-050">Ce qui manque encore : les ventes et la population.</strong> Ce tableau montre ce qu&apos;on <em>demande</em> pour un exemplaire gradé, pas ce qu&apos;il s&apos;est <em>vendu</em>, et ignore combien d&apos;exemplaires existent dans chaque grade. Sans population, aucun ratio PSA / brute n&apos;est publié — un écart de prix sans rareté relative ne veut rien dire.</aside></section>
